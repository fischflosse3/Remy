...
const productName = process.env.PRODUCT_NAME || 'Remy Tab AI';
...
app.get('/', (_req, res) => {
  res.setHeader('Content-Type', 'text/html; charset=utf-8');
  res.end('<!doctype html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Remy Tab AI</title><style>body{font-family:system-ui,sans-serif;background:#f8fafc;color:#111827;display:grid;place-items:center;min-height:100vh;margin:0}.card{background:white;border-radius:24px;padding:28px;box-shadow:0 20px 60px #0001;max-width:460px}a{display:inline-block;margin-top:14px;background:#4f46e5;color:white;padding:12px 16px;border-radius:14px;text-decoration:none;font-weight:800}</style></head><body><main class="card"><h1>Remy Tab AI Backend läuft</h1><p>Diese Seite ist nur der Server hinter Remy Tab AI. Für Login öffne die Remy-Erweiterung oder nutze die Login-Seite.</p><a href="/auth">Zum Login</a></main></body></html>');
});
...
app.post('/api/auth/delete', requireUser, async (req, res) => {
  try {
    const eligibility = await getAccountDeletionEligibility(req.user);
...
app.post('/api/ask', requireUser, async (req, res) => {
  try {
    ...
    const context = mode === 'local' ? memories.map((m, i) => {
      ...
      return [
        `Quelle ${i + 1}:`,
        `Quelle-Art: ${isOpenTab ? 'Aktuell offener Tab (nur Titel + URL, kein Seiteninhalt)' : 'Gespeicherte Browser-Erinnerung'}`,
        `Titel: ${clip(m.title, 180)}`,
        `Domain: ${clip(m.domain, 120)}`,
        `URL: ${clip(m.url, 400)}`,
        `Gespeichert/Status: ${clip(m.savedAt, 80)}`,
        m.platform ? `Plattform: ${clip(m.platform, 80)}` : '',
        m.searchQuery ? `Suchanfrage: ${clip(m.searchQuery, 160)}` : '',
        `Kurzfassung: ${clip(m.summary, 700)}`,
        isOpenTab ? 'Textauszug: Nicht verfügbar. Der Webseiteninhalt wurde nicht ausgelesen.' : `Textauszug: ${clip(m.text, 2500)}`
      ].filter(Boolean).join('\n');
...
app.post('/api/create-customer-portal-session', requireUser, async (req, res) => {
  try {
    ...
    const customerId = await getOrCreateStripeCustomerId(req.user, false);
    if (!customerId) return res.status(400).json({ error: 'Für dieses Konto wurde noch kein Stripe-Kunde gefunden. Bitte prüfe, ob du mit derselben E-Mail bezahlt hast.' });
...
async function getAccountDeletionEligibility(user) {
  ...
  // Lifetime ist kein laufendes Stripe-Abo. Falls du Lifetime später verkaufst,
  // kann der Nutzer sein Konto trotzdem löschen, weil keine monatliche Abbuchung offen ist.
  if (plan === 'lifetime') return { ok: true };

  if (plan !== 'plus') return { ok: true };
  if (!stripe) return { ok: false, error: 'Abo-Status konnte nicht geprüft werden. Bitte Stripe im Backend konfigurieren.' };

  const customerId = await getOrCreateStripeCustomerId(user, false);
  if (!customerId) {
    // Kein Stripe-Kunde gefunden: Dann gibt es auch kein aktives Stripe-Abo, das vor Löschung geschützt werden muss.
    return { ok: true };
  }

  const subscriptions = await stripe.subscriptions.list({ customer: customerId, status: 'all', limit: 100 });
  const blocking = subscriptions.data.find((sub) => {
    const statusBlocksDeletion = ['active', 'trialing', 'past_due', 'unpaid', 'incomplete'].includes(sub.status);
    const alreadyCancels = Boolean(sub.cancel_at_period_end || sub.cancel_at || sub.canceled_at || sub.status === 'canceled');
    return statusBlocksDeletion && !alreadyCancels;
  });

  if (blocking) {
    return {
      ok: false,
      error: 'Du hast noch ein aktives Unlimited-Abo. Bitte kündige es zuerst über „Abo verwalten“. Danach kannst du dein Konto sofort löschen, auch wenn das Abo noch bis zum Periodenende läuft.'
    };
  }
...
function renderAuthPage(deviceId) {
  return `<!doctype html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Remy Tab AI Login</title><style>
