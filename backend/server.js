...
app.post('/api/auth/delete', requireUser, async (req, res) => {
  const isPaid = req.user.plan === 'plus' || req.user.plan === 'lifetime';
  const isCancelled = req.user.cancel_at_period_end === true || req.user.subscription_status === 'canceled' || req.user.subscription_status === 'cancelled';
  if (isPaid && !isCancelled) {
    return res.status(400).json({ error: 'Du hast einen aktiven Plan. Bitte kündige dein Abo zuerst über Stripe. Danach kannst du dein Konto löschen.' });
  }
...
app.post('/api/create-customer-portal-session', requireUser, async (req, res) => {
  try {
    ...
    if (!customerId && req.user.email) {
      ...
      if (customerId) await updateUserBilling(req.user.id, { stripe_customer_id: customerId });
    }
    if (!customerId) return res.status(400).json({ error: 'Für dieses Konto wurde kein Stripe-Abo gefunden.' });
    const session = await stripe.billingPortal.sessions.create({ customer: customerId, return_url: `${publicBaseUrl}/auth?billing=1` });
    res.json({ ok: true, url: session.url });
  } catch (error) {
...
app.post('/api/stripe/webhook', async (req, res) => {
  try {
    ...
    if (type === 'checkout.session.completed') {
      ...
      if (userId) await updateUserBilling(userId, { plan: 'plus', stripe_customer_id: obj.customer || null, stripe_subscription_id: obj.subscription || null, subscription_status: 'active', cancel_at_period_end: false });
    }
    ...
    if (type === 'customer.subscription.updated') {
      await updateSubscriptionStatus(obj);
    }
...
function publicUser(user) { return { id: user.id, email: user.email, plan: user.plan || 'free', subscription_status: user.subscription_status || null, cancel_at_period_end: Boolean(user.cancel_at_period_end) }; }
...
async function markSubscriptionFree(subscriptionId) {
  if (supabase) {
    try {
      const { data } = await supabase.from('remy_users').select('id').eq('stripe_subscription_id', subscriptionId).maybeSingle();
      if (data?.id) await updateUserBilling(data.id, { plan: 'free', stripe_subscription_id: null, subscription_status: 'canceled', cancel_at_period_end: true });
      return;
    } catch (e) { console.warn('Supabase subscription cancel failed', e.message); }
  }
  const s = await readStore();
  const user = Object.values(s.users).find(u => u.stripe_subscription_id === subscriptionId);
  if (user) { user.plan = 'free'; user.stripe_subscription_id = null; user.subscription_status = 'canceled'; user.cancel_at_period_end = true; await writeStore(s); }
}
...
async function updateSubscriptionStatus(subscription) {
  ...
  const patch = {
    ...
    plan: subscription.status === 'canceled' || subscription.status === 'unpaid' ? 'free' : 'plus'
  };
  if (supabase) {
    try {
      const { data } = await supabase.from('remy_users').select('id').eq('stripe_subscription_id', subscriptionId).maybeSingle();
      if (data?.id) await updateUserBilling(data.id, patch);
      return;
    } catch (e) { console.warn('Supabase subscription status update failed', e.message); }
  }
  const store = await readStore();
  const user = Object.values(store.users).find(u => u.stripe_subscription_id === subscriptionId);
  if (user) { Object.assign(user, patch); await writeStore(store); }
}
