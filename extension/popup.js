...
let popupChatState = { chats: [], activeChatId: null };
let popupActiveChatId = null;
...
async function init(){
  ...
  $('chatPicker').onclick = async()=>{ await loadPopupChats(); $('chatList').classList.toggle('hidden'); };
  $('newChat').onclick = async()=>{ const r=await send({type:'REMY_NEW_CHAT',mode:state.mode}); if(r?.ok){popupChatState=r; popupActiveChatId=r.activeChatId; renderPopupChats(); $('answer').innerHTML='<span class="ai-label">Neuer Chat</span> Folgefragen bleiben jetzt in diesem Thema.';} };
...
async function refreshAll(){ const [st, auth] = await Promise.all([send({type:'OMNI_GET_STATE'}), refreshAuth()]); if(st?.ok){state={...state,...st};} render(); await checkBackend(); if(state.loggedIn){ await refreshUsage(); await loadPopupChats(); } }
...
function setMode(mode){ state.mode=mode==='public'?'public':'local'; send({type:'REMY_SET_MODE',mode:state.mode}); renderMode(); loadPopupChats(); $('question').focus(); }
async function ask(question){ question=String(question||'').trim(); if(!question)return; $('answer').innerHTML='<span class="ai-label">Remy denkt…</span>'; const r=await send({type:'REMY_SIDEBAR_ASK',question,mode:state.mode,chatId:popupActiveChatId}); if(!r?.ok){$('answer').textContent=r?.error||'Remy konnte nicht antworten.'; if(r?.loginRequired) send({type:'REMY_START_LOGIN'}); return;} if(r.usage){state.usage=r.usage;renderUsage();} if(r.activeChatId) popupActiveChatId=r.activeChatId; if(r.chats){popupChatState={chats:r.chats,activeChatId:r.activeChatId||popupActiveChatId}; renderPopupChats();} $('answer').innerHTML=`<span class="ai-label">Antwort</span>\n${escapeHtml(r.answer||'Keine Antwort.')}${renderSources(r.sources||[])}`; bindLinks(); $('question').value=''; }
...
function renderMode(){ const pub=state.mode==='public'; $('modeLocal').classList.toggle('active',!pub); $('modePublic').classList.toggle('active',pub); $('modeHelp').textContent=pub?'Allgemein fragen nutzt allgemeines KI-Wissen. Keine privaten Daten eingeben.':'Browser suchen nutzt deine gespeicherten Browser-Erinnerungen.'; }
...
async function loadPopupChats(){
  ...
  const r=await send({type:'REMY_GET_CHAT_STATE',mode:state.mode});
  if(r?.ok){ popupChatState=r; popupActiveChatId=r.activeChatId; renderPopupChats(); }
}
function renderPopupChats(){
  const list=$('chatList'); if(!list) return;
  const chats=popupChatState.chats||[];
  list.innerHTML=chats.length?chats.slice(0,6).map(c=>`<button type="button" class="chat-option ${c.id===popupActiveChatId?'active':''}" data-id="${escapeHtml(c.id)}">${escapeHtml(c.title || (state.mode==='public'?'Allgemeiner Chat':'Browser-Chat'))}</button>`).join(''):'<div class="chat-option">Noch kein Chat</div>';
  list.querySelectorAll('.chat-option[data-id]').forEach(btn=>btn.onclick=async()=>{const r=await send({type:'REMY_SELECT_CHAT',mode:state.mode,chatId:btn.dataset.id}); if(r?.ok){popupChatState=r; popupActiveChatId=r.activeChatId; renderPopupChats(); list.classList.add('hidden'); $('answer').innerHTML='<span class="ai-label">Chat gewechselt</span> Du kannst hier weiterfragen.';}});
}
